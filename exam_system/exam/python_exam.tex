...
\item%
你要克服地心引力跨越大海飞向太空，为了实现这个梦想，你首先要下载具有该功能的\autolenunderline{} (如"antigravity"或者"flyhigh"), 然后在 Python 的脚本文件中使用\autolenunderline{}关键字导入;
%
\item%
...
\item%
视频和图像属于\autolenunderline{}文件，用\autolenunderline{}方法写入（保存或另存为），用\autolenunderline{}方法读取;%
\item%
Python脚本文件后缀是\autolenunderline{};%
\item%
如果函数没有返回语句，那么它默认范围: \autolenunderline{};%
\item%
...
\item%
至少写出两个个你熟悉的标准方库: \autolenunderline{}、\autolenunderline{};%
\item%
...
\item%
Python 的 for/while 循环语句和 if 语句一样可以接 else;~~\mypar{}%
\item%
Python 有 do-while 语句;~~\mypar{}%
\item%
...
\item%
通过\verb|from PIL import Image|导入包之后，下面可以做的是;
~~\mypar{}\\
(A) 使用PIL~~(B) 使用Image~~(C) 为Image赋值~~(D) 为PIL赋值%
\item%
下面哪个是不合法的命令;~~\mypar{}\\
