...
\item%
C 语言中的结构体类似于 Python 中的\autolenunderline[mask]{`dict`}类型;%
\item%
...
\item%
如果函数没有返回语句，那么它默认返回: \autolenunderline[mask]{None};%
\item%
循环语句中，Python 的控制命令有: \autolenunderline[mask]{break}、\autolenunderline[mask]{continue};%
\item%
读取文件内容前，一般用\autolenunderline[mask]{open}创建文件对象;%
\item%
文件操作结束时候要执行\autolenunderline[mask]{close}命令;%
\item%
创建文件对象的常用语法糖是\autolenunderline[mask]{with}语句;%
\item%
定义类方法的关键字是：\autolenunderline[mask]{def};%
\item%
Python 网络爬虫中堪称绝配的两个第三方库/模块: \autolenunderline[mask]{`requests`}、\autolenunderline[mask]{`bs4`};%
\item%
