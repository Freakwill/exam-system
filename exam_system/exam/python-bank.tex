...
\item%
下面两处\verb|print|命令打印出的结果分别是\autolenunderline{Hello}、\autolenunderline{HelloWorld};
...
\item%
设类\verb|Hero|中定义了方法\verb|attack|，\verb|hero|是\verb|Hero|的对象。正确调用这个方法的命令是;~~\mypar{A}\\
...
\end{lstlisting}
~~\mypar{D}\\
(A) 最后打印结果为Hello RUC~~(B) 第一个\verb|x|为全局变量~~(C) \verb|f()|中的\verb|x|为全局变量~~(D) \verb|f()|中的\verb|x|为局部变量%
\item%
