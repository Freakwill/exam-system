...
paper.calculation = []
paper.fill = (FillProblem @ code).read_yaml(filename='python_fill')
paper.choice = (ChoiceProblem @ code).read_yaml(filename='python_choice')
paper.truefalse = (TrueFalseProblem @ replace_backticks_with_verb).read_yaml(filename='python_truefalse')
paper.calculation = (CalculationProblem @ code).read_yaml(filename='python_calculation')
paper.build()
